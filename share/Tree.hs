...
getTreeLen :: MonadIO m => Tree -> m Int
getTreeLen tree = liftIO $ fmap length $ H.toList $ unTree tree

treeLookup :: MonadIO m => Tree -> RawFilePath -> m (Maybe TreeContent)
treeLookup tree filename = liftIO $ H.lookup (unTree tree) filename 
